...
GRIN is competitive rather than dominant. For forecasting, we compare eight
methods and find a clean split by paradigm: at horizons of 6~hours or more, all
three spatiotemporal graph models beat all four temporal methods, including the
transformer state of the art (PatchTST, iTransformer), which are no better than a
per-station autoregression. The decisive factor is spatial information, not
temporal architecture, and the advantage holds across the graph family (16--20\%
skill over persistence at 24~hours). We trace this advantage to its physical cause:
the network's wave-height cross-correlations recover the deep-water dispersion relation
(observed swell lags match the group-velocity prediction $d/c_g$ at
$\rho=0.80$), so the graph's real edge is that it reads swell propagation.
...
\begin{itemize}
  ...
  \item We benchmark eight forecasters, from persistence and autoregression to
  transformer SOTA (PatchTST, iTransformer) and three spatiotemporal graph models,
  and show that the graph family forms a distinct upper tier at horizons of 6~h or
  more, while the temporal models, however sophisticated, cluster together far
  below.
  \item We give a \emph{physical explanation} for that advantage: the buoy network
  recovers the deep-water dispersion relation (cross-correlation lags track the
  group-velocity prediction $d/c_g$, $\rho=0.80$), we map the basin-scale swell
  corridors it implies, and we show the graph's edge is its ability to use an upstream
  buoy as a causal preview of arriving swell.
...
a wave-period ratio consistent with the empirical $T_e/T_{02}$ values reported by
\citet{cahill2014};\footnote{This treats NDBC's average wave period (APD) as
equivalent to the average zero-crossing period $T_{02}$ used in that ratio, an
assumption to confirm with co-authors, not an established identity.} $\alpha$ is
an explicit configuration parameter. True energy uses observed WVHT and APD at
...
\noindent\textbf{Forecasting models.} Besides persistence and the per-station
AR(24), we evaluate three temporal deep models, DLinear \citep{dlinear2023},
PatchTST \citep{patchtst2023} and iTransformer \citep{itransformer2024}, and three
spatiotemporal graph models, Graph WaveNet \citep{gwn}, DCRNN \citep{dcrnn} and
AGCRN \citep{agcrn2020}. All are direct multi-horizon forecasters: they read a
24-hour input window across all stations and predict 24 hours ahead in one shot, so
every shorter horizon is read from the same forecast. All share the leakage-safe
protocol (trained on the GRIN-completed 2021--2024 tensor, scored on causal
forward-filled test inputs at the same origins as the baselines).

\noindent\textbf{Hyperparameters and compute.} Deep models are seeded
(seed~0) and checkpointed, with best weights saved on every validation improvement.
GRIN is trained with a capped budget (up to 50 epochs, early stopping with patience
10). All forecasters are trained on CPU: Graph WaveNet's channels-last convolution
is unsupported on the Apple MPS backend, and we observed that sustained MPS load
both degrades throughput and, for the full-tensor imputation pass, exhausts GPU
memory, so CPU is used for reproducibility. For compute tractability the transformer
and graph forecasters are trained on a 1-in-3 subsample of forecast origins (capped
at 20 epochs); DLinear and the classical baselines use every origin. Adjacent hourly
origins are highly correlated, so this subsampling has little effect on the fitted
models. All runs use a fixed software stack (\texttt{tsl}~\citep{tsl},
\texttt{PyPOTS}~\citep{pypots}, PyTorch~\citep{pytorch}, PyTorch~Geometric~\citep{pyg}).
...
purely temporal deep models (SAITS 0.730~m, BRITS 0.725~m)
...
gaps are easy to interpolate, so the temporal methods lead. Linear interpolation
(WVHT 0.074~m) is best by a hair over the attention model SAITS
(0.075~m), and GRIN (0.082~m) is competitive but behind, the
graph adds little when time alone suffices. The message is not that GRIN dominates
everywhere, but that it is best exactly where spatial or temporal structure must be
exploited, and merely competitive where a gap is trivial to fill.
...
Linear interp & 0.637 & 1.303 & \textbf{0.074} & 0.211 & 0.557 & 1.186 \\
Spatial IDW & 0.443 & 0.765 & 0.430 & 0.844 & 0.437 & 0.889 \\
Spatial+temporal & 0.426 & 0.846 & 0.229 & 0.462 & 0.392 & 0.857 \\
SAITS & 0.730 & 1.826 & 0.075 & \textbf{0.209} & 0.595 & 0.827 \\
BRITS & 0.725 & 1.565 & 0.124 & 0.351 & 0.600 & 0.898 \\
GRIN & \textbf{0.346$\pm$0.007} & \textbf{0.726$\pm$0.033} & 0.082$\pm$0.002 & 0.228$\pm$0.002 & \textbf{0.373$\pm$0.021} & \textbf{0.704$\pm$0.039} \\
\bottomrule
\end{tabular}}
\end{table}

\paragraph{Robustness across missingness rates.}
Tables~\ref{tab:sweepW} and~\ref{tab:sweepA} extend the comparison to five
missingness rates (5--50\%) for both the random (MCAR) and contiguous-block
regimes, so the ranking is not read off a single rate. The pattern is stable
across the whole range. Under MCAR, where a hidden hour usually sits between two
observed neighbours in time, the temporal methods win at every rate: the attention
model SAITS is best at almost every rate (linear interpolation edges it only at the
highest WVHT rate). GRIN trails the temporal leaders here, and the spatial-only
interpolators trail furthest. Time, not space, is the easy axis. Under contiguous
blocks the ranking reverses cleanly: GRIN is best at every block rate for \emph{both}
targets, because a longer temporal gap is exactly where a station must borrow from
its neighbours. The single-paradigm floor and the graph's advantage are therefore
properties of the regime, not artefacts of one masking rate.

\begin{table}[t]
\centering
\caption{Imputation MAE for \textbf{WVHT} (m) across missingness rates, MCAR and
Block at 5/10/20/30/50\%. GRIN is the basin-aware graph (mean over three model
seeds). Lower is better; best per column in bold.}
\label{tab:sweepW}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{l ccccc ccccc}
\toprule
& \multicolumn{5}{c}{MCAR} & \multicolumn{5}{c}{Block}\\
\cmidrule(lr){2-6}\cmidrule(lr){7-11}
Method & 5\% & 10\% & 20\% & 30\% & 50\% & 5\% & 10\% & 20\% & 30\% & 50\%\\
\midrule
Mean fill & 0.476 & 0.476 & 0.476 & 0.476 & 0.476 & 0.470 & 0.474 & 0.465 & 0.451 & 0.445 \\
Forward fill & 0.087 & 0.088 & 0.092 & 0.097 & 0.111 & 0.557 & 0.599 & 0.586 & 0.628 & 0.702 \\
Linear interp & 0.065 & 0.065 & 0.067 & 0.068 & \textbf{0.074} & 0.460 & 0.520 & 0.505 & 0.504 & 0.557 \\
Spatial IDW & 0.371 & 0.376 & 0.387 & 0.399 & 0.430 & 0.368 & 0.366 & 0.373 & 0.381 & 0.437 \\
Spatial+temporal & 0.200 & 0.202 & 0.207 & 0.213 & 0.229 & 0.337 & 0.351 & 0.357 & 0.354 & 0.392 \\
SAITS & \textbf{0.052} & \textbf{0.053} & \textbf{0.057} & \textbf{0.062} & 0.075 & 0.628 & 0.643 & 0.639 & 0.618 & 0.595 \\
BRITS & 0.079 & 0.082 & 0.089 & 0.098 & 0.124 & 0.688 & 0.687 & 0.667 & 0.635 & 0.600 \\
GRIN & 0.065 & 0.066 & 0.068 & 0.071 & 0.082 & \textbf{0.325} & \textbf{0.333} & \textbf{0.339} & \textbf{0.341} & \textbf{0.373} \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{table}[t]
\centering
\caption{Imputation MAE for \textbf{APD} (s) across missingness rates, MCAR and
Block at 5/10/20/30/50\%. GRIN is the basin-aware graph (mean over three model
seeds). Lower is better; best per column in bold.}
\label{tab:sweepA}
\small
...
Method & 5\% & 10\% & 20\% & 30\% & 50\% & 5\% & 10\% & 20\% & 30\% & 50\%\\
\midrule
Mean fill & 0.997 & 0.998 & 0.999 & 0.997 & 0.997 & 1.002 & 0.989 & 0.981 & 0.969 & 0.968 \\
Forward fill & 0.240 & 0.244 & 0.255 & 0.268 & 0.306 & 1.202 & 1.213 & 1.258 & 1.281 & 1.410 \\
Linear interp & 0.177 & 0.179 & 0.184 & 0.191 & 0.211 & 1.059 & 1.084 & 1.094 & 1.087 & 1.186 \\
Spatial IDW & 0.752 & 0.760 & 0.777 & 0.795 & 0.844 & 0.768 & 0.772 & 0.783 & 0.789 & 0.889 \\
Spatial+temporal & 0.408 & 0.412 & 0.422 & 0.433 & 0.462 & 0.761 & 0.770 & 0.778 & 0.785 & 0.857 \\
SAITS & \textbf{0.148} & \textbf{0.152} & \textbf{0.162} & \textbf{0.174} & \textbf{0.209} & 0.819 & 0.823 & 0.835 & 0.827 & 0.827 \\
BRITS & 0.215 & 0.225 & 0.249 & 0.276 & 0.351 & 0.947 & 0.934 & 0.928 & 0.913 & 0.898 \\
GRIN & 0.178 & 0.181 & 0.189 & 0.198 & 0.228 & \textbf{0.616} & \textbf{0.615} & \textbf{0.627} & \textbf{0.640} & \textbf{0.704} \\
\bottomrule
\end{tabular}}
\end{table}
...
basin-aware graph against the plain $k$-nearest-neighbour graph; in 24
of 24 target-regime cells the difference is smaller than the combined
...
\subsection{Swell propagation and the graph mechanism}
\label{sec:propagation}
Why do the graph models win? We show the reason is physical: the buoy network encodes
swell propagation, and the graph's advantage is the ability to exploit it. Ocean swell
travels between buoys at the deep-water group velocity $c_g = gT/4\pi$, so a wave event
at an upstream buoy reaches a downstream buoy after a lag $\tau = d/c_g$. For every
within-basin buoy pair we take the lag that maximises the cross-correlation of their
significant-wave-height series and compare it to this prediction
(Figure~\ref{fig:prop}). The observed lags track the physics closely: Spearman
$\rho=0.80$ over 1{,}427 within-basin pairs, and 99\% of pairs respect
the causal bound $|\tau^{*}|\le d/c_g$, so the correlation almost never propagates faster
than the group velocity. The agreement is coherent within every basin and strongest along
the West Coast ($\rho=0.89$), where long Pacific swell dominates. The network has
recovered the deep-water dispersion relation from data alone.

The lead-lag structure defines a directed \emph{swell-propagation graph}
(Figure~\ref{fig:corridors}): 758 upstream-to-downstream edges over 133
of the 140 buoys, with a median propagation lag of 5~hours. These are
data-derived maps of basin-scale swell corridors.

This propagation is the mechanism behind the two-tier forecasting result below. To
forecast a buoy $h$ hours ahead, an upstream buoy roughly $h$ hours away is a
\emph{causal preview}: its already-observed state is the swell that will arrive at the
target. It explains why forecast skill \emph{grows with horizon}
(Figure~\ref{fig:horizon}), since longer horizons make more distant upstream buoys usable
as previews, and, in a controlled test, propagation-lag-aligned upstream features improve
24~h WVHT forecasts over same-time neighbour features. The spatial graph wins because it
reads the wave field the way the ocean actually propagates it.

\begin{figure}[t]
\centering
\includegraphics[width=0.66\textwidth]{fig12_propagation.png}
\caption{The buoy network recovers wave propagation. For within-basin pairs, the observed
cross-correlation lag $|\tau^{*}|$ tracks the deep-water group-velocity prediction
$d/c_g$; points colour by coupling strength. Nothing lies above the diagonal: the
correlation does not outrun the group velocity.}
\label{fig:prop}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{fig13_swell_corridors.png}
\caption{Data-derived swell-propagation corridors. Each arrow runs from the upstream
(leading) buoy to the downstream buoy of a strongly-coupled within-basin pair, coloured by
propagation lag and drawn with width proportional to coupling strength.}
\label{fig:corridors}
\end{figure}

\subsection{Forecasting}
Table~\ref{tab:fc} reports forecast MAE and persistence-normalized skill across
five horizons, from 1 to 24 hours, for eight methods: persistence and a
per-station autoregression (AR(24)); three strong temporal deep models (DLinear,
PatchTST, iTransformer); and three spatiotemporal graph models (DCRNN, AGCRN, and
Graph WaveNet). Three findings stand out.

First, persistence is extremely strong at very short range: at 1~hour its WVHT
error is only 0.086~m, and the best learned model improves on it by only
+0.039, so we report honestly that a learned model adds little at 1~hour. All
skill grows with horizon as the sea state has time to evolve.

Second, and most important, the methods separate into two clean tiers by
\emph{paradigm}. At every horizon of 6~hours or more, all three graph models beat
all three temporal deep models and the autoregression. At 24~hours no temporal
method exceeds +0.097 WVHT skill, while no graph model falls below
+0.159: the tiers do not overlap. The strongest temporal SOTA models,
PatchTST and iTransformer, are no better than the classical AR(24). The decisive
ingredient is spatial information, not temporal architecture; a model that can read
a buoy's neighbours forecasts it far better than one that cannot, however
sophisticated its treatment of time.

Third, the graph advantage is a property of the graph \emph{family}, not of one
architecture. Graph WaveNet (fixed $k$-NN graph), DCRNN (diffusion on that graph),
and AGCRN (a \emph{learned} adjacency) all reach the upper tier. AGCRN is best at
long range (+0.199 WVHT skill at 24~hours) but worst at 1~hour (-0.040,
below persistence): a learned graph needs a longer context to pay off.
Figure~\ref{fig:horizon} shows the two tiers and the widening gap as the horizon
grows.

\begin{table}[t]
\centering
\caption{Forecast MAE and persistence-normalized skill (in parentheses) at
horizons of 1 to 24 hours. Persistence is the reference, so its skill is zero by
definition. Higher skill is better.}
\label{tab:fc}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{l ccccc ccccc}
\toprule
& \multicolumn{5}{c}{WVHT (m)} & \multicolumn{5}{c}{APD (s)}\\
\cmidrule(lr){2-6}\cmidrule(lr){7-11}
Method & $h{=}1$ & $h{=}3$ & $h{=}6$ & $h{=}12$ & $h{=}24$
& $h{=}1$ & $h{=}3$ & $h{=}6$ & $h{=}12$ & $h{=}24$\\
\midrule
Persistence & 0.086 & 0.138 & 0.205 & 0.296 & 0.416 & 0.236 & 0.386 & 0.550 & 0.724 & 0.884 \\
AR(24) & 0.083 (+0.038) & 0.133 (+0.039) & 0.197 (+0.042) & 0.280 (+0.054) & 0.378 (+0.090) & 0.229 (+0.030) & 0.369 (+0.045) & 0.511 (+0.072) & 0.654 (+0.096) & 0.789 (+0.107) \\
DLinear & 0.085 (+0.017) & 0.135 (+0.020) & 0.200 (+0.024) & 0.285 (+0.039) & 0.386 (+0.071) & 0.233 (+0.012) & 0.380 (+0.015) & 0.535 (+0.028) & 0.692 (+0.043) & 0.828 (+0.063) \\
PatchTST & 0.083 (+0.030) & 0.133 (+0.038) & 0.195 (+0.048) & 0.277 (+0.064) & 0.379 (+0.089) & 0.231 (+0.018) & 0.375 (+0.029) & 0.525 (+0.046) & 0.676 (+0.066) & 0.815 (+0.078) \\
iTransformer & 0.085 (+0.010) & 0.133 (+0.038) & 0.194 (+0.053) & 0.274 (+0.074) & 0.375 (+0.097) & 0.232 (+0.014) & 0.372 (+0.037) & 0.517 (+0.061) & 0.663 (+0.084) & 0.805 (+0.089) \\
GraphWaveNet & 0.083 (+0.039) & 0.124 (+0.103) & 0.177 (+0.138) & 0.249 (+0.158) & 0.347 (+0.166) & 0.223 (+0.055) & 0.349 (+0.096) & 0.478 (+0.132) & 0.607 (+0.161) & 0.745 (+0.157) \\
DCRNN & 0.084 (+0.019) & 0.126 (+0.091) & 0.179 (+0.128) & 0.252 (+0.150) & 0.350 (+0.159) & 0.230 (+0.024) & 0.352 (+0.088) & 0.478 (+0.130) & 0.605 (+0.163) & 0.748 (+0.153) \\
AGCRN & 0.089 (-0.040) & 0.129 (+0.067) & 0.181 (+0.119) & 0.248 (+0.163) & 0.333 (+0.199) & 0.240 (-0.016) & 0.359 (+0.070) & 0.482 (+0.124) & 0.595 (+0.177) & 0.716 (+0.189) \\
\bottomrule
\end{tabular}}
\end{table}
...
\caption{Forecast skill versus horizon for the eight methods. The three
spatiotemporal graph models (Graph WaveNet, DCRNN, AGCRN) form an upper tier that
separates from the four temporal methods (AR(24), DLinear, PatchTST, iTransformer)
at every horizon of 6~hours or more; the gap widens with horizon. AGCRN's learned
graph overtakes at 24~hours but is weakest at 1~hour.}
\label{fig:horizon}
\end{figure}
...
observed, at every horizon from 1 to 24 hours. The two-tier split from the
forecasts not only persists but \emph{widens} in energy space. At 24~hours every
graph model exceeds +0.226 energy skill while no temporal method reaches
+0.173, and AGCRN leads at +0.266. Graph WaveNet's own energy skill
rises with horizon to +0.217 at $h{=}12$ and +0.235 at $h{=}24$, larger
than its height and period skill. The reason is the $H^{2}$ nonlinearity in
...
parentheses) at horizons of 1 to 24 hours, scored where both WVHT and APD are
observed. Rankings and skills are invariant to the period coefficient $\alpha$.}
\label{tab:energy}
\small
...
Method & $h{=}1$ & $h{=}3$ & $h{=}6$ & $h{=}12$ & $h{=}24$\\
\midrule
Persistence & 1.752 & 2.579 & 3.764 & 5.468 & 7.691 \\
AR(24) & 1.658 (+0.054) & 2.441 (+0.054) & 3.489 (+0.073) & 4.863 (+0.111) & 6.434 (+0.164) \\
DLinear & 1.697 (+0.031) & 2.486 (+0.036) & 3.569 (+0.052) & 4.972 (+0.091) & 6.581 (+0.144) \\
PatchTST & 1.671 (+0.046) & 2.433 (+0.057) & 3.450 (+0.083) & 4.792 (+0.124) & 6.402 (+0.168) \\
iTransformer & 1.748 (+0.002) & 2.473 (+0.041) & 3.475 (+0.077) & 4.778 (+0.126) & 6.358 (+0.173) \\
GraphWaveNet & 1.669 (+0.048) & 2.278 (+0.117) & 3.113 (+0.173) & 4.284 (+0.217) & 5.882 (+0.235) \\
DCRNN & 1.665 (+0.050) & 2.289 (+0.112) & 3.148 (+0.164) & 4.348 (+0.205) & 5.949 (+0.226) \\
AGCRN & 1.811 (-0.034) & 2.383 (+0.076) & 3.192 (+0.152) & 4.274 (+0.219) & 5.648 (+0.266) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.82\textwidth]{fig11_energy_forecast.png}
\caption{Wave-energy forecast skill versus horizon. The two-tier split by paradigm
carries into energy and widens: the graph models pull further ahead of the temporal
methods than in the height/period forecasts, because power scales with the square of
wave height.}
\label{fig:energy}
\end{figure}
