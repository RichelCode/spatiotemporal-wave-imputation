...
have valid neighbours, so its sample size ($n\approx38$) is smaller than the
full panel.
...
is small ($n\approx38$ stations), so we read this as \emph{suggestive},
...
($n\approx38$), so these are suggestive.}
\label{tab:conn}
\small
...
$n\approx38$ stations it is underpowered, and we label it suggestive.
