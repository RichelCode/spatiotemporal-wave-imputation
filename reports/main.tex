...
five horizons, from 1 to 24 hours. Two findings stand out. First, persistence is
extremely strong at very short range: at 1~hour its WVHT error is only
0.086~m, and the best learned model improves on it by only +0.039, so we
report honestly that a learned model adds little at 1~hour. Second, that value
grows monotonically with horizon. Graph WaveNet's WVHT skill climbs from +0.039
at 1~hour to +0.138 at 6~hours and +0.166 at 24~hours, with a similar climb on
APD (+0.055 to +0.157). At 12--24~hours this is roughly double the
autoregressive model, whose WVHT skill is +0.054 at $h{=}12$ and +0.090 at
$h{=}24$. Figure~\ref{fig:horizon} shows how persistence degrades and how the
learned models open a widening gap as the horizon grows.
...
observed, at every horizon from 1 to 24 hours. The graph model's advantage not
only persists but \emph{grows}. Its energy-forecast skill rises with horizon to
+0.217 at $h{=}12$ and +0.235 at $h{=}24$, larger than its own height and
period skill, while the autoregressive model reaches +0.111 and +0.164 at
those horizons. The reason is the $H^{2}$ nonlinearity in
