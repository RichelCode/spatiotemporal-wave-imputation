...
is limited to the subset of stations that both host a station-outage victim and
have valid neighbours, so its sample size ($n\approx@nw@$) is smaller than the
full panel.
...
\caption{Distributions of the target variables, significant wave height (WVHT)
and average wave period (APD).}
\label{fig:tdist}
\end{figure}
...
is small ($n\approx@nw@$ stations), so we read this as \emph{suggestive},
directional evidence rather than a firm law. Figure~\ref{fig:conn} visualizes the
relationship.
...
``advantage'' is GRIN minus SAITS. $^{*}p<0.05$. Sample is small
($n\approx@nw@$), so these are suggestive.}
\label{tab:conn}
\small
...
$n\approx@nw@$ stations it is underpowered, and we label it suggestive.
