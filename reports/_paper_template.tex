...
height (WVHT) and average wave period (APD) over @n_stations@ stations selected
...
under whole-station outages it recovers WVHT at @grin_so_wvht@~m mean absolute
error (MAE), well below spatial interpolation (@idw_so_wvht@~m), and it is also
...
$\rho=@prop_rho@$), so the graph's real edge is that it reads swell propagation.
...
reaches @en_gwn24@ versus persistence, wider than its height skill. Across all
...
\begin{itemize}
  ...
  group-velocity prediction $d/c_g$, $\rho=@prop_rho@$), we map the basin-scale swell
...
stripping NDBC missing-value sentinels before counting. From @n_audit@ audited
stations we selected @n_stations@ that meet a coverage-and-longevity threshold
over 2021--2025, yielding an hourly tensor of @n_hours@ time steps across the
station panel used throughout (Figure~\ref{fig:map}). The connectivity analysis
in Section~\ref{sec:results}
is limited to the station-outage victim stations, of which there are @nw@ with a
scorable WVHT recovery and @na@ with a scorable APD recovery (five victims do not
...
@grin_so_wvht@~m, far below the best spatial baseline (IDW, @idw_so_wvht@~m) and
the spatial-plus-temporal interpolator (@st_so_wvht@~m), and well below the
purely temporal deep models (SAITS @saits_so_wvht@~m, BRITS @brits_so_wvht@~m)
...
(WVHT @lin_mcar_wvht@~m) is best by a hair over the attention model SAITS
(@saits_mcar_wvht@~m), and GRIN (@grin_mcar_wvht@~m) is competitive but behind, the
...
stations, GRIN reduces station-outage error by about @adv_w@~m in WVHT and
@adv_a@~s in APD relative to SAITS. This gap is the graph's specific
...
is small ($n=@nw@$ victim stations for WVHT and $n=@na@$ for APD), so we read this
...
($n=@nw@$ stations for WVHT, $n=@na@$ for APD), so these are suggestive.}
\label{tab:conn}
\small
...
basin-aware graph against the plain $k$-nearest-neighbour graph; in @abl_within@
of @abl_total@ target-regime cells the difference is smaller than the combined
...
$\rho=@prop_rho@$ over @prop_n@ within-basin pairs, and @prop_bound@\% of pairs respect
...
the West Coast ($\rho=@prop_wc_rho@$), where long Pacific swell dominates. The network has
...
(Figure~\ref{fig:corridors}): @prop_edges@ upstream-to-downstream edges over @prop_nodes@
of the @n_stations@ buoys, with a median propagation lag of @prop_medlag@~hours. These are
...
error is only @pers1_wvht@~m, and the best learned model improves on it by only
@gwn1_w@, so we report honestly that a learned model adds little at 1~hour. All
...
method exceeds @temp_max24@ WVHT skill, while no graph model falls below
@graph_min24@: the tiers do not overlap. The strongest temporal SOTA models,
...
long range (@agcrn24_w@ WVHT skill at 24~hours) but worst at 1~hour (@agcrn1_w@,
...
graph model exceeds @en_graph_min24@ energy skill while no temporal method reaches
@en_temp_max24@, and AGCRN leads at @en_agcrn24@. Graph WaveNet's own energy skill
rises with horizon to @en_gwn12@ at $h{=}12$ and @en_gwn24@ at $h{=}24$, larger
...
$n=@nw@$ victim stations for WVHT (and $n=@na@$ for APD) it is underpowered, and
