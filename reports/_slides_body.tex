...
\begin{frame}{Station selection: a documented cascade}
  ...
  \begin{tabular}{r l}
    ...
    Stage & Count \\
    \midrule
    Wave-plausible stations (crawl) & @v_plausible@ \\
    Audited with computed coverage & \textbf{@n_audit@} \\
    Have any WVHT observations & \textbf{@n_wave@} \\
    Retained: $\ge$70\% WVHT coverage, $\ge$3 years & \textbf{@n_sel@} \\
    \bottomrule
  \end{tabular}
  \end{center}
  \take{The filter is a fixed rule, not a hand-pick: $\ge$70\% coverage over $\ge$3 years $\rightarrow$ @thr140@ stations.}
...
\begin{frame}{The retained network spans every U.S. basin}
  \begin{center}\includegraphics[width=\textwidth,height=0.64\textheight,keepaspectratio]{fig1_station_map.png}\end{center}
...
\begin{frame}{The data as one tensor}
  \begin{center}\large
  $[\,@n_hours@\ \text{hours}\;\times\;@n_sel@\ \text{stations}\;\times\;3\ \text{features}\,]$\\[1ex]
...
\begin{frame}{Missingness is structured, not random}
  \begin{center}\includegraphics[width=\textwidth,height=0.60\textheight,keepaspectratio]{fig2_missingness.png}\end{center}
...
\begin{frame}{Preprocessing: three decisions, each justified}
  \begin{itemize}
    \item \textbf{log1p on WVHT only} --- right-skewed (skew $\approx$~@v_skew@); periods are near-symmetric, kept in natural units.
    \item \textbf{DPD, APD in natural units} --- no transform needed.
    \item \textbf{Wind excluded} --- @v_anem@ of @n_sel@ stations have no anemometer: structurally missing-not-at-random, so wind is dropped rather than imputed.
  \end{itemize}
...
\begin{frame}{Why a graph? Nearby buoys inform each other}
  \begin{center}\includegraphics[width=\textwidth,height=0.60\textheight,keepaspectratio]{fig7_corr_distance.png}\end{center}
...
\begin{frame}{The dichotomy --- the hinge of the talk}
  ...
  \begin{tabular}{l cc}
    ...
    \midrule
    \onslide<2->{\textcolor{colspatial}{Spatial IDW}} & \onslide<2->{\textbf{@idw_so_w@}} & \onslide<2->{@idw_mc_w@} \\
    \onslide<3->{\textcolor{coltemporal}{SAITS (temporal)}} & \onslide<3->{@saits_so_w@} & \onslide<3->{\textbf{@saits_mc_w@}} \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vfill
...
\begin{frame}{GRIN wins both regimes}
  ...
  \begin{tabular}{l cc}
    ...
    \midrule
    \textcolor{colspatial}{Spatial IDW} & @idw_so_w@ & @idw_mc_w@\\
    \textcolor{coltemporal}{SAITS} & @saits_so_w@ & @saits_mc_w@\\
    \textcolor{colgraph}{\textbf{GRIN}} & \textbf{@grin_so_w@} & \textbf{@grin_mc_w@}\\
    \bottomrule
  \end{tabular}
  \end{center}
...
\begin{frame}{Mechanism: isolating the graph's contribution}
  ...
  \begin{tabular}{l ccc}
    ...
    \midrule
    WVHT (m, $n=$@nvic_w@) & @grin_pv_w@ & @saits_pv_w@ & \textbf{@advw@}\\
    APD (s, $n=$@nvic_a@) & @grin_pv_a@ & @saits_pv_a@ & \textbf{@adva@}\\
    \bottomrule
  \end{tabular}
  \end{center}
  \take{$\sim$@advw@\,m WVHT and $\sim$@adva@\,s APD --- attributable to the graph alone.}
...
@TAB_ABL@
  ...
  \end{tabular}
  \end{center}
  \take{All @abl_within@/@abl_total@ config$\times$target cells within model-seed noise --- a useful simplification.}
...
@TAB_CONN@
    \bottomrule
  \end{tabular}\\[0.5ex]
  {\scriptsize $^{*}p<0.05$; Spearman $\rho$. $n=$@nconn_w@ (WVHT), @nconn_a@ (APD).}
  \end{center}
  \take{Signs match the hypothesis, several are significant --- but $n\approx$@nconn_w@ is underpowered. We do not overclaim.}
...
@TAB_FC@
  ...
  \end{tabular}}
  \end{center}
  \take{Graph WaveNet $\sim$16\% skill throughout --- AR(24) only @ar_lo@--@ar_hi@.}
...
\begin{frame}{Skill grows with horizon}
  \begin{center}\includegraphics[width=\textwidth,height=0.66\textheight,keepaspectratio]{fig10_horizon_skill.png}\end{center}
...
@TAB_ENERGY@
  ...
  \end{center}
  \take{Graph skill @en_gwn12@/@en_gwn24@ --- wider than its height skill: the $H^2$ nonlinearity amplifies it.}
...
\begin{frame}{Limitations --- volunteered}
  \begin{enumerate}\small
    \item Connectivity analysis underpowered ($n\approx$@nconn_w@).
...
\begin{frame}{Reproducibility}
  \begin{itemize}\small
    ...
    \item An uncapped 100-epoch run scored marginally better (@v_discard@\,$\approx$0.331) but was \emph{discarded} for irreproducibility --- a deliberate trade.
  \end{itemize}
...
\begin{frame}{Backup: additional EDA figures}
  \begin{columns}
    \column{0.5\textwidth}\centering\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{fig3_coverage_dist.png}
    \column{0.5\textwidth}\centering\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{fig5_temporal_cycles.png}
  \end{columns}
\end{frame}

\begin{frame}{Backup: distributions and connectivity}
  \begin{columns}
    \column{0.5\textwidth}\centering\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{fig4_target_dists.png}
    \column{0.5\textwidth}\centering\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{fig9_connectivity_recovery.png}
  \end{columns}
\end{frame}

\begin{frame}{Backup: energy forecast skill}
  \begin{center}\includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{fig11_energy_forecast.png}\end{center}
\end{frame}
